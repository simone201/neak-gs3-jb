...
#define MAX_CURRENT_AC   1200
#define MAX_CURRENT_CDP 1200
#define MAX_CURRENT_USB  1200
#define MAX_CURRENT_DOCK 1200
int charge_current_ac = 1000;
int charge_current_cdp = 1000;
int charge_current_usb = 475;
int charge_current_dock = 1000;
...
static ssize_t charge_current_store(struct device *dev, struct device_attribute *attr, const char *buf,
	...
	if (ret != 4) {
		...
		charge_current_ac = (temp[0]<MAX_CURRENT_AC) ? temp[0] : MAX_CURRENT_AC;
		charge_current_cdp = (temp[1]<MAX_CURRENT_CDP) ? temp[1] : MAX_CURRENT_CDP;
		charge_current_usb = (temp[2]<MAX_CURRENT_USB) ? temp[2] : MAX_CURRENT_USB;
		charge_current_dock = (temp[3]<MAX_CURRENT_DOCK) ? temp[3] : MAX_CURRENT_DOCK;
	}
	return count;	
}
